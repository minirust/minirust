...
/// Helper which builds code to assert that compute_align with given type gives the expected align.
...
fn mismatched_meta_ill_formed() {
    ...
    assert_ill_formed::<BasicMem>(
        p,
        "UnOp::ComputeSize|ComputeAlign: invalid right type: not metadata of type",
    );
}
