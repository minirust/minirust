...
The function `pick<T>(impl Distribution, fn(T) -> bool) -> Nondet<T>` will return a value of type `T` such that the given closure returns `true` for this value.
...
This argument does not affect the set of possible program behaviors, it is purely a hint for the interpreter.
