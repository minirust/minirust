...
```rust
impl Type {
    fn decode<M: Memory>(Type::Tuple { sized_fields, sized_head_layout, unsized_field: None }: Self, bytes: List<AbstractByte<M::Provenance>>) -> Option<Value<M>> {
    ...
    fn encode<M: Memory>(Type::Tuple { sized_fields, sized_head_layout, unsized_field: None }: Self, val: Value<M>) -> List<AbstractByte<M::Provenance>> {
        let (size, _) = sized_head_layout.head_size_and_align();
...
impl Type {
    ...
    fn decode<M: Memory>(Type::Tuple { unsized_field: Some(_), .. }: Self, bytes: List<AbstractByte<M::Provenance>>) -> Option<Value<M>> {
        panic!("decode of Type::Tuple with unsized field")
    }
    fn encode<M: Memory>(Type::Tuple { unsized_field: Some(_), .. }: Self, val: Value<M>) -> List<AbstractByte<M::Provenance>> {
        panic!("decode of Type::Tuple with unsized field")
    }
}
```
...
impl<M: Memory> Machine<M> {
    ...
    fn check_value(&self, value: Value<M>, ty: Type) -> Result {
        match (value, ty) {
            ...
            (Value::Tuple(vals), Type::Tuple { sized_fields, unsized_field: None, .. }) => {
            ...
            (_, Type::Tuple { unsized_field: Some(_), .. }) => panic!("Value: unsized structs cannot be represented as values"),
